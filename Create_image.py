...
def hash_to_decimal_to_coord(filename): #  Function that reads a file line by line, converts each hash to two decimal numbers and returns a list of tuples
    ...
    for line in lines:
        ...
        decimalx = int(hash_string[:3], 16) / 10    # Convert first 3 characters of hash to decimal to get x coordinate
        decimaly = int(hash_string[-3:], 16) / 10   # Convert last 3 characters of hash to decimal to get y coordinate
        decimal_list.append((decimalx, decimaly))   # Creating a list of with x and y
    
    return decimal_list

def show_pixel_supperposition(decimal_list):  # Function to show pixel superpositions
    for x, y in decimal_list:                  
        ...
        if 0 <= x < 409 and 0 <= y < 409:               # S'assurer que x et y sont dans les limites
            ...
            if current_color == (255, 255, 255):    # If it's white
                im.set_pixel(x, y, (0, 255, 0))     # Green dot is set in case of no collision
                ...
                im.set_pixel(x, y, (255, 0, 0))     # Green dot is set if no collision
                    ...
                    im.set_pixel(x, y, (0, 0, 255))  # Change to blue dot for multiple collisions
            
im = image(409, 409, "white")                           # Create a blank image of size 409x409 with white background
decimal_list = hash_to_decimal_to_coord("sha1rand.txt")     # Get the list of decimal coordinates from the file
show_pixel_supperposition(decimal_list)                     # Show pixel superpositions     
